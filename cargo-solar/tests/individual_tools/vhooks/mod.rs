...
pub fn assert_configuration(path: &Path, name: &str, assert_true: bool) {
    let solar_config = Config::load_from(path).unwrap();
    let vhooks_config = solar_config.vhooks().as_ref().unwrap();
    assert_eq(vhooks_config.name(), &name.to_string(), assert_true);
}

pub fn assert_installation(path: &Path, name: &str, assert_true: bool) {
    assert!(fs::exists(path.join(".git")).unwrap());
    assert(fs::exists(path.join(name)).unwrap(), assert_true);
    assert_eq(
