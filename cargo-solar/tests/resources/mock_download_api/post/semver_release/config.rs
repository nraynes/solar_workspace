...
use crate::resources::mock_download_api::{LATEST, db::SEMVER_RELEASE_CONFIG_FILE};

pub fn config(req: Request, mut db: RefMut<Database>) -> Response {
    let new_file = req.body();
    if let Some(table) = db.table_mut(SEMVER_RELEASE_CONFIG_FILE) {
