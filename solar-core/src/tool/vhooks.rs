...
impl ToolTrait for Vhooks {
    fn install(&self) -> Result<(), SolarError> {
        ...
        Terminal::command().piped().run("mkdir", ["-p", &hooks_path])?;
    ...
    fn uninstall(&self) -> Result<(), SolarError> {
        ...
        Terminal::command().piped().run("mkdir", ["-p", &default_path])?;

        // If not removing hooks, move them to the default hooks directory.
        if !self.remove_all {
            Terminal::command().piped().run("mv", [&hooks_path, &default_path])?;
        }
        ...
        Terminal::command().piped().run("rm", ["-rf", &hooks_path])?;
